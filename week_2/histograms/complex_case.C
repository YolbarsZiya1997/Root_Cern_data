void complex_case() {
    ...
    TCanvas *cst = new TCanvas("cst", "stacked hists", 10, 10, 1500, 900);
    cst->Divide(2,2);
    // in top left pad, draw the stack with defaults
    cst->cd(1);
    hs->Draw();
    // in top right pad, draw the stack with non-stack mode
    // and errors option
    cst->cd(2);
    gPad->SetGrid();
    hs->Draw("nostack, elp");
